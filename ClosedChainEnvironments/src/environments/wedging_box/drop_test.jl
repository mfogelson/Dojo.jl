...
contacts = contact_constraint(mechanism.bodies[1], normals;
        ...
        contact_origins = contact_origins, contact_radii = fill(0.0, 8),
    )
...
function controller!(mechanism, k)
    ...
    if k == 1
        ...
        add_external_force!(mechanism.bodies[1], force=[100*external_force, 0.0, 0.0], vertex=[0.0, 0.0, edge_length/2])
    end
end
