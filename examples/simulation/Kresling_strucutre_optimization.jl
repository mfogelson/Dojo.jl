...
function optimize_configuration(; VEHICAL_NAME, DIAMETER, MASS, INITIAL_HEIGHT, EXPANDED_HEIGHT, STATES=5, DENSITY=2710.0)
    ...
    @NLconstraint(model, r == lBC[1])
    @NLconstraint(model, r == lBC[STATES])
 
    @NLconstraint(model, lAC[1] <= DIAMETER)
    @NLconstraint(model, phi[STATES] == deg2rad(-10.0))
    ...
    volume = @NLexpression(model, m*(6*(2*thickness)^2*(lAC[1]+lBC[1])))
    ...
    CSV.write("/Users/mitchfogelson/Downloads/$(VEHICAL_NAME)_Kresling_Only_initialState_mass.csv", df, writeheader=false)
    ...
    # Save the modified DataFrame to a new CSV file
    CSV.write("/Users/mitchfogelson/Downloads/$(VEHICAL_NAME)_Kresling_Only_finalState_mass.csv", df, writeheader=false)
    ...
    # changes = Dict("l1" => value(l1), "l2" => value(l2), "l3" => value(l3), "l2l3" => (value(l3)+value(l2)), "thickness" => value(thickness)*1000.0, "alpha" => (value(α[1])), "beta" => (value(β[1])))

    # # Apply the changes
    # update_dataframe!(df, :name, :value, changes)

    # # Save the modified DataFrame to a new CSV file
    # CSV.write("/Users/mitchfogelson/Downloads/$(VEHICAL_NAME)_PET_initialState.csv", df, writeheader=false)

    # changes = Dict("l1" => value(l1), "l2" => value(l2), "l3" => value(l3), "l2l3" => (value(l3)+value(l2)), "thickness" => value(thickness)*1000.0, "alpha" => (value(α[STATES])), "beta" => (value(β[STATES])))

    # # Apply the changes
    # update_dataframe!(df, :name, :value, changes)

    # # Save the modified DataFrame to a new CSV file
    # CSV.write("/Users/mitchfogelson/Downloads/$(VEHICAL_NAME)_PET_finalState.csv", df, writeheader=false)

    # TODO: Fix this Save model to file 
    # write_to_file(model, "model_$VEHICAL_NAME.mof.json")
    keys = [:thickness, :r, :phi, :h, :m]
    for key in keys
