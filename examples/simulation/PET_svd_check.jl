...
num_dof = 6*length(mechanism.bodies)
...
using FiniteDiff
hes = FiniteDiff.finite_difference_jacobian(x -> cost(mechanism, x), input)
...
attjac = cat(data_attitude_jacobian_config.(mechanism.bodies)..., dims=(1,2))
hes_new = hes*attjac

F = svd(collect(hes_new), full=true)
rank = sum(F.S .> 1e-6)
println(rank)
V1 = @view F.V[:,1:rank]
S1 = @view F.S[1:rank]
