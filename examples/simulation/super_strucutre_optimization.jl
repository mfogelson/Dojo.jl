...
function optimize_configuration(; VEHICAL_NAME, DIAMETER, MASS, INITIAL_HEIGHT, EXPANDED_HEIGHT, STATES=5, DENSITY=2.710)
    ...
    volume = @NLexpression(model, 12*m*(2*thickness)^2*((2*l1)*8*n + 2*(2*l3+l2)*n))
...
optimize_configuration(; input_params_sls_b2...)
