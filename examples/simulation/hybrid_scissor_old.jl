...
function get_kresling(kres_radius, beam_length, diag_length, unit_width, unit_thickness, unit_length, unit_mass; timestep=0.01)
    ...
    for i in 1:6
        ...
        append!(bodies, body)
        append!(joints, joint)
        top_joint = JointConstraint(Spherical(body[end-1], top; parent_vertex=[0, 0, unit_length/2], child_vertex=[kres_radius*cos(2pi/6*(i-1)), kres_radius*sin(2pi/6*(i-1)), -unit_thickness/2]), name=Symbol("Spherical_Joint_Beam_to_Top$(i)"))
        push!(joints, top_joint)

        bottom_joint = JointConstraint(Spherical(base, body[1]; parent_vertex=[kres_radius*cos(2pi/6*(i-1)), kres_radius*sin(2pi/6*(i-1)), unit_thickness/2], child_vertex=[0, 0, -unit_length/2]), name=Symbol("Spherical_Joint_Base_to_Beam$(i)"))
        ...
        append!(bodies, body)
        append!(joints, joint)
        top_joint = JointConstraint(Spherical(body[end], top; parent_vertex=[0, 0, unit_length/2], child_vertex=[kres_radius*cos(2pi/6*i), kres_radius*sin(2pi/6*i), -unit_thickness/2]), name=Symbol("Spherical_Joint_Long_Beam_to_Top$(i)"))
        push!(joints, top_joint)

        bottom_joint = JointConstraint(Spherical(base, body[2]; parent_vertex=[kres_radius*cos(2pi/6*(i-1)), kres_radius*sin(2pi/6*(i-1)), unit_thickness/2], child_vertex=[0, 0, -unit_length/2]), name=Symbol("Spherical_Joint_Base_to_Long_Beam$(i)"))
        push!(joints, bottom_joint)
    end
...
function initialize_kresling!(mechanism, kres_radius, beam_length, diag_length, unit_width, unit_thickness, unit_length)
    ...
    set_maximal_configurations!(mechanism.bodies[2], x=[0, 0, height+unit_thickness], q=Dojo.RotZ(0))
    for i in 1:6
        number_of_units = floor(Int, beam_length/(unit_width))
        ...
        for j in 1:number_of_units
            ...
            x = Dojo.rotation_matrix(Dojo.RotZ(0))*[(kres_radius+unit_length/2)*cos((i-1)*pi/3), (kres_radius+unit_length/2)*sin((i-1)*pi/3), (1-t)*unit_thickness/2 + t*(height+unit_thickness/2)]
        ...
        for j in 1:number_of_units
            t = (j-1) / (number_of_units-1)
            x = [(1-t)*(kres_radius-unit_length/2)*cos((i-1)*pi/3) + t*(kres_radius-unit_length/2)*cos((i)*pi/3), (1-t)*(kres_radius-unit_length/2)*sin((i-1)*pi/3) + t*(kres_radius-unit_length/2)*sin((i)*pi/3), (1-t)*unit_thickness + t*(height+unit_thickness/2)]
...
initialize_constraints!(mechanism, fixedids=[mechanism.bodies[1].id, mechanism.bodies[2].id], regularization=1e-10, lineIter=10, newtonIter=100)
...
function control!(mechanism, k)
    add_external_force!(mechanism.bodies[2], force=[0, 0, 0.], torque=[0, 0, 0.])
end
# for i in 1:length(mechanism.joints)
#     mechanism.system.matrix_entries[i,i].value += Dojo.I
# end
@time storage = simulate!(mechanism, 0.5, control!, record=false, opts=SolverOptions(verbose=true))
